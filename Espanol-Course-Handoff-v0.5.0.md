...
**Handoff version:** v0.5.0 — Current as of 2026-03-16
...
### C1 unit list (locked — not yet built)
...
### Versioning
- **Semver:** v0.5.0 → v0.6.x (C1) → v1.0.0 (public launch)
- **Folder structure:** `/content/C1/unit-01-advanced-grammar/`
...
│       │       └── ExerciseSummary.jsx
│       └── pages/
│           ├── CourseMapPage.jsx   ← A1–B2 fully wired (10+12+12+12 units)
│           ├── LessonPage.jsx
│           ├── ExercisesPage.jsx
│           ├── QuizPage.jsx
│           ├── FlashcardsPage.jsx
...
- ✅ Sequential unlock across all levels: A1 → A2 → B1 → B2 gated correctly
...
Always: `"LEVEL-unit-XX-slug"` — e.g. `"B2-unit-09-literature"`
...
```
Where `level` is uppercase (A1, A2, B1, B2, C1) and `unit` is the full folder name (e.g. `unit-01-advanced-grammar`).

**Unit IDs must match folder names exactly** in `CourseMapPage.jsx`:
- ✅ Correct: `id: 'unit-01-advanced-grammar'` → folder: `unit-01-advanced-grammar`
- ❌ Wrong: `id: 'unit-01'` → folder: `unit-01-advanced-grammar` (causes 404)
...
- **C1 Unit 1 (when built): gate on `'B2-unit-12-review'`**

### Adding C1
1. Create content in `/content/C1/unit-XX-slug/` and `/app/public/content/C1/unit-XX-slug/`
2. Add C1 level object to `COURSE_STRUCTURE` in `CourseMapPage.jsx` with `comingSoon: false`
3. Add unlock gate: `if (levelData.level === 'C1' && index === 0)` → check `'B2-unit-12-review'`
4. Use color: `'from-orange-500 to-amber-500'`


**Exact C1 unlock gate code to add inside the `isAvailable` block in `CourseMapPage.jsx`:**
```jsx
...
```
This block goes after the `B2 && index === 0` block and before the `index === 0` fallback.
...
```json
{
  ...
  "example_sentence": "...",
  "example_translation": "...",
  "audio_file": "C1/unit-01/example.mp3",
  "tags": ["..."]
}
```

### exercises.json item — MCQ (integer answer required)
```json
{
  "id": "C1-U01-E001",
...
## Integrity check prompt for previous levels

If you ever need to verify the exercise/quiz schema integrity of A1, A2, or B1 content, paste this into that level's build session:
...
Claude must run this Python script (or equivalent) before zipping any content. Copy-paste and execute in bash_tool:

```python
import json, os, re
from collections import Counter

BASE = "/home/claude/build/content/C1"  # adjust path as needed
...
|---------|------|-------------|
| v0.5.0 | 2026-03-16 | B2 Units 9–12 complete — B2 now fully done (12/12 units) |
| v0.4.2 | 2026-03-16 | B2 Units 5–8 (vocabulary, media, argumentation, register) |
| v0.4.1 | 2026-03-16 | B2 Units 1–4 (past subjunctive, si clauses, connectors) |
...
2. Upload `Espanol-Course-Handoff-v0.5.0.md` as a file attachment
