...
**Handoff version:** v0.3.0 — Current as of 2026-03-15
...
### Versioning
- **Semver:** v0.1.x → v0.2.x → v0.3.0 → v1.0.0
...
│   │   └── content/
│   │       ├── A1/
│   │       │   └── unit-01-greetings/ through unit-10-review/  ← 10 units, 5 files each
│   │       └── A2/
│   │           └── unit-01-present-tense/ through unit-12-review/  ← 12 units, 5 files each
│   └── src/
│       ├── App.jsx
│       ├── main.jsx
│       ├── styles/globals.css
...
│       │       └── ExerciseSummary.jsx
│       └── pages/
│           ├── CourseMapPage.jsx   ← A1 (10 units) + A2 (12 units) fully wired
│           ├── LessonPage.jsx
│           ├── ExercisesPage.jsx
│           ├── QuizPage.jsx
│           ├── FlashcardsPage.jsx
...
│   └── A2/
│       └── unit-01-present-tense/ through unit-12-review/
...
- ❌ B1–C1 content not written (next major milestone)
- ❌ Audio files — TTS generation not run yet (needs Google Cloud account setup)
- ❌ Placement test not built (planned for end of v1.0)
...
Examples: `"A1-unit-01-greetings"`, `"A2-unit-06-imperfect"`, `"B1-unit-01-subjunctive"`
...
When delivering a zip, Claude must include both locations. The user unzips and merges.

### CourseMapPage.jsx — level unlock logic
- A1 Unit 1 is always available (no prereq)
- A1 Units 2–10 unlock sequentially after the previous quiz is passed
- A2 Unit 1 unlocks only after A1 Unit 10 is complete (`'A1-unit-10-review'`)
- A2 Units 2–12 unlock sequentially
- When adding B1, apply the same pattern: B1 Unit 1 gates on `'A2-unit-12-review'`

### Adding a new level (e.g. B1)
1. Create content files in `/content/B1/unit-XX-slug/` and `/app/public/content/B1/unit-XX-slug/`
2. Add the level object to `COURSE_STRUCTURE` in `CourseMapPage.jsx` with `comingSoon: false`
3. Add the unlock gate: `if (levelData.level === 'B1' && index === 0)` → check `'A2-unit-12-review'`
4. Remove `comingSoon: true` from the B1 entry
...
1. **B1 content** — 12 units. Suggested topics:
   - unit-01-subjunctive-intro — Introduction to the Subjunctive
   - unit-02-future-tense — The Future Tense
   - unit-03-conditional — The Conditional Tense
   - unit-04-por-vs-para — Por vs. Para
   - unit-05-relative-clauses — Relative Clauses (que, quien, donde)
   - unit-06-passive-voice — Passive Voice & Se Constructions
   - unit-07-idiomatic-verbs — Idiomatic Verbs (gustar, doler, faltar, etc.)
   - unit-08-opinion-debate — Expressing Opinions & Debate
   - unit-09-formal-writing — Formal Writing & Email
   - unit-10-mexican-culture — Mexican Culture & Traditions
   - unit-11-regional-language — Regional Language & Slang
   - unit-12-review — B1 Review & Checkpoint
2. **Fix deprecated meta tag** — one line in `app/index.html`, change `apple-mobile-web-app-capable` to `mobile-web-app-capable`
...
|---------|------|-------------|
| v0.3.0 | 2026-03-15 | A2 Units 9–12 content complete — A2 now fully done (12/12 units) |
| v0.2.2 | 2026-03-15 | A2 Units 9–12 content + final CourseMapPage with all 12 A2 units |
| v0.2.1 | 2026-03-15 | A2 Units 5–8 content (preterite irregular, imperfect, preterite vs imperfect, object pronouns) |
...
2. Upload `Espanol-Course-Handoff-v0.3.0.md` as a file attachment
