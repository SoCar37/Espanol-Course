...
**Handoff version:** v0.4.0 — Current as of 2026-03-15
...
### B2 unit list (locked — not yet built)
...
### Versioning
- **Semver:** v0.1.x → v0.2.x → v0.3.x → v0.4.0 → v0.5.0 → v1.0.0
- **Folder structure:** `/content/B1/unit-01-subjunctive-intro/` (lowercase, hyphenated, zero-padded)
...
├── README.md
├── app/
│   ├── index.html              ← ⚠️ still has deprecated apple-mobile-web-app-capable meta tag
│   ├── package.json
│   ├── postcss.config.js
│   ├── tailwind.config.js
│   ├── vite.config.js
...
│       │       └── ExerciseSummary.jsx
│       └── pages/
│           ├── CourseMapPage.jsx   ← A1 (10) + A2 (12) + B1 (12) fully wired
│           ├── LessonPage.jsx
│           ├── ExercisesPage.jsx
│           ├── QuizPage.jsx
│           ├── FlashcardsPage.jsx
...
- ✅ Sequential unlock across levels: A1 → A2 → B1 gated correctly
...
- ❌ B2–C1 content not written (next major milestone: B2)
- ❌ Audio files — TTS generation not run yet (needs Google Cloud account setup)
- ❌ Placement test not built (planned for end of v1.0)
- ❌ Deprecated meta tag in index.html (`apple-mobile-web-app-capable` → fix manually: change to `mobile-web-app-capable`)
...
Examples: `"A1-unit-01-greetings"`, `"A2-unit-06-imperfect"`, `"B1-unit-01-subjunctive-intro"`
...
1. `/content/B1/unit-XX-slug/` — source of truth
2. `/app/public/content/B1/unit-XX-slug/` — served copy
...
- **B2 Unit 1 (when built): gate on `'B1-unit-12-review'`**

### Adding B2
1. Create content in `/content/B2/unit-XX-slug/` and `/app/public/content/B2/unit-XX-slug/`
2. Add B2 level object to `COURSE_STRUCTURE` in `CourseMapPage.jsx` with `comingSoon: false`
3. Add unlock gate: `if (levelData.level === 'B2' && index === 0)` → check `'B1-unit-12-review'`
4. Remove `comingSoon: true` from the B2 entry

---

## Next steps (in priority order)

1. **B2 content** — 12 units. Unit list locked (see above). Use same 4-per-session approach.
2. **Fix deprecated meta tag** — in `app/index.html`, manually change `apple-mobile-web-app-capable` to `mobile-web-app-capable`
3. **Audio** — requires user to set up Google Cloud TTS account
...
```json
{
  ...
  "topics": ["subjunctive mood", "present subjunctive"],
  "prereqs": ["A2-U12"],
...
```json
{
  "id": "B1-U01-V001",
  "word": "ojalá",
  "translation": "hopefully / I hope",
  "pronunciation_hint": "oh-hah-LAH",
  "part_of_speech": "interjection (always triggers subjunctive)",
  "example_sentence": "Ojalá que todo salga bien.",
  "example_translation": "Hopefully everything turns out well.",
  "audio_file": "B1/unit-01/ojala.mp3",
  "tags": ["subjunctive", "essential", "mexico"]
}
```
...
**For sentence_assembly, use `"words"` not `"options"`:**
```json
{
  "id": "B1-U01-E008",
  "type": "sentence_assembly",
  "prompt": "Arrange the words...",
  "words": ["Ojalá", "que", "todo", "salga", "bien."],
  "answer": "Ojalá que todo salga bien.",
  "explanation": "..."
}
```
...
|---------|------|-------------|
| v0.4.0 | 2026-03-15 | B1 Units 9–12 complete — B1 now fully done (12/12 units) |
| v0.3.2 | 2026-03-15 | B1 Units 5–8 (relative clauses, passive se, idiomatic verbs, opinions) |
| v0.3.1 | 2026-03-15 | B1 Units 1–4 (subjunctive, future, conditional, por/para) |
...
2. Upload `Espanol-Course-Handoff-v0.4.0.md` as a file attachment
